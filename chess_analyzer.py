...
class ChessGameAnalyzer:
    ...
    def create_training_data(self, username, analyzed_games, weaknesses, platform="auto"):
    ...
    def analyze_player(self, username, platform="auto"):
        ...
        if not games:
            print("❌ No games downloaded. Please check the username and platform.")
            return False
        
        # Step 2: Parse PGN files
        parsed_games = self.parse_pgn_files()
        if not parsed_games:
            print("❌ No games parsed from PGN files")
            return False
        ...
        training_data = self.create_training_data(username, analyzed_games, weaknesses, platform)
...
def main():
    ...
    print(f"\n🎯 Analyzing {username} on {platform}")
    print("=" * 50)
    ...
    if success:
        ...
        # Show which PGN files were created
        if os.path.exists("chess_com_games.pgn"):
            print(f"📄 Chess.com games saved to: chess_com_games.pgn")
        if os.path.exists("lichess_games.pgn"):
            print(f"� Lichess games saved to: lichess_games.pgn")
            
        print(f"�💡 Run the web UI to generate strategies!")
    else:
        print(f"\n❌ Analysis failed for {username}")
        print("💡 Try checking the username or try a different platform")
